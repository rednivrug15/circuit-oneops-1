    ...
    class WebSite
      class WebApp
        ...
        def delete_virtual_directory path
          reload
          if virtual_directory_exists? path
            @virtual_directory_collection.DeleteElement(get_virtual_directory_position(path))
          else
            Chef::Log.info "The virtual directory #{path} does not exist. Please make sure that the virtual directory exists."
          end
        end
